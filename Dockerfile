...
#
# NOTE: This Dockerfile runs as root. Use it on VPS / Docker hosts where
# root is allowed. For PaaS platforms (Render, StackShift, Koyeb free plans)
# that require non-root containers, use the Alpine + non-root variant in
# git history (commit a60edf2 or later).
